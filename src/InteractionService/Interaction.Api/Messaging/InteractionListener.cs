...
public class InteractionListener : BackgroundService
{
    
    private readonly RabbitMQSettings _settings;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<InteractionListener> _logger;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            ...
            try
            {
                var @event = JsonSerializer.Deserialize<UserInteractionEvent>(json, new JsonSerializerOptions
                {
                ...
                var saveUserInteraction = scope.ServiceProvider.GetRequiredService<SaveUserInteraction>();
                await saveUserInteraction.ExecuteAsync(@event);
